...
function CheckLogin() {
	...
	if(form.pw.value.length < 4) {
		...
		form.pw.select();	// 비밀번호 입력란으로 포커스를 옮겨주기
		return false;
	}
	form.submit();		// 정상적으로 입력했다면 form action에 있는 경로로 이동하게 해준다.
}

$(function(){
	var submitBtn2 = $("#submitBtn2");	// 전송버튼 Element
	...
	submitBtn2.on("click", function() {
		...
		if(pw2.length < 4 || pw2.length > 12) {
			alert("비밀번호는 4자 이상 입력 가능합니다!");
			$("#pw2").select();	// 비밀번호 입력란으로 포커스를 옮겨주기
			return false;
		}
		loginForm2.submit();
	});
